...
echo "Setting up the Python environment and edv command..."
...
if [[ "$CONFIRM" != "y" ]]; then
    echo "Setup abortedv. Please move the script to the correct location or update the repository path."
    exit 1
fi

# Define the directory for the Python environment in the "edv/setup" subdirectory
ENV_DIR="$REPO_ROOT/edv/setup/edv_venv"

# Create the "edv/setup" directory if it doesn't exist
mkdir -p "$REPO_ROOT/edv/setup"
...
REQUIREMENTS_FILE="$REPO_ROOT/edv/requirements.txt"
echo "Installing dependencies from $REQUIREMENTS_FILE..."

# Check if the requirements.txt file exists
if [ ! -f "$REQUIREMENTS_FILE" ]; then
    echo "ERROR: Could not find the requirements.txt file at $REQUIREMENTS_FILE"
    echo "Please ensure the file exists in the 'setup' subdirectory of 'edv'."
    exit 1
fi
...
if [[ "$SHELL" == *"zsh"* ]]; then
    ...
    echo "Detectedv zsh shell. Using $PROFILE_FILE for environment variables."
elif [[ "$SHELL" == *"bash"* ]]; then
    PROFILE_FILE="$HOME/.bashrc"
    echo "Detectedv bash shell. Using $PROFILE_FILE for environment variables."
else
...
echo "Adding EDV_ROOT environment variable to $PROFILE_FILE..."

# Add MONARCH_ROOT environment variable to the user's shell configuration
echo "export EDV_ROOT=\"$REPO_ROOT\"" >> $PROFILE_FILE

# Create a simple wrapper script for the command namedv 'edv'
EDV_COMMAND="$REPO_ROOT/edv/setup/edv"
echo "Creating the edv command at $EDV_COMMAND..."
...
echo "Adding the edv command to your PATH in $PROFILE_FILE..."

# Add the script to the user's PATH in their shell configuration
echo "export PATH=\"$REPO_ROOT/edv/setup:\$PATH\"" >> $PROFILE_FILE
...
echo "You can now use the 'edv' command to run your script."
